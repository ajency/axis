...
if ( ! function_exists( 'hb_generate_dynamic_css' ) ) {
	function hb_generate_dynamic_css() {
		...
		.hb-single-next-prev a:hover {
			background: '. hb_color($focus_hex_color, 0.95).';	
		}

		.hb-more-details:hover, .hb-buy-button:hover {
			color:#FFF;
			background: '. hb_color($focus_hex_color, 0.8).';
		}
		...
		if ( hb_options('hb_color_manager_type') == 'hb_color_manager_color_customizer' ){
			...
			if (get_theme_mod('hb_top_bar_border_setting')){
				...
				$css .= '#header-bar .top-widget { border-right-color: '. get_theme_mod('hb_top_bar_border_setting') .'!important; border-left-color: '.get_theme_mod('hb_top_bar_border_setting').'; }';
			}
			...
			if (get_theme_mod('hb_nav_bar_stuck_bg_setting')){
				$css .= '#header-inner.stuck #header-inner-bg { background-color: '.get_theme_mod('hb_nav_bar_stuck_bg_setting').'; }';
			}

			if (get_theme_mod('hb_side_nav_bg_setting')){
				$css .= '#hb-side-navigation.hb-non-transparent, #hb-side-navigation .hb-resp-bg { background-color: '.get_theme_mod('hb_side_nav_bg_setting').'; }';
			}

			if (get_theme_mod('hb_nav_bar_border_setting')){
				$css .= '#header-inner.nav-type-2 .main-navigation { border-top-color: '.get_theme_mod('hb_nav_bar_border_setting').'; }';
			...
			if (get_theme_mod('hb_nav_bar_stuck_border_setting')){
				$css .= '#header-inner.stuck #header-inner-bg { border-bottom-color:'.get_theme_mod('hb_nav_bar_stuck_border_setting').' !important; }';
				$css .= '#header-inner.stuck #main-nav li#nav-search::before {background: '.get_theme_mod('hb_nav_bar_stuck_border_setting').'}';
			}
			...
			if (get_theme_mod('hb_nav_bar_text_setting')){
				...
				$css .= 'body:not(.page-template-page-presentation-fullwidth) #main-wrapper #main-nav > li > a, #main-wrapper #header-inner-bg { color:'.get_theme_mod('hb_nav_bar_text_setting').' !important; }';

			}

			if (get_theme_mod('hb_nav_bar_stuck_text_setting')){
				$css .= '#main-wrapper #header-inner.stuck #main-nav > li > a, #main-wrapper #header-inner.stuck #header-inner-bg { color:'.get_theme_mod('hb_nav_bar_stuck_text_setting').' !important; }';
			}
			...
			if (get_theme_mod('hb_spec_head_color_setting')){
				$css .= '.hb-special-header-style #main-wrapper #header-inner #main-nav > li > a, 
					  ...
					  .hb-special-header-style #close-fancy-search:hover { color: '.get_theme_mod('hb_spec_head_color_setting').' !important; }';
			...
			if (get_theme_mod('hb_spec_head_bg_stuck_setting')){
				$css .= '.hb-special-header-style #main-wrapper #header-inner.stuck #header-inner-bg { background: '.hb_color( get_theme_mod('hb_spec_head_bg_stuck_setting'), 0.7).' !important; border-bottom: 0; }';
			}

			if (get_theme_mod('hb_spec_head_color_sticky_setting')){
				$css .= '.hb-special-header-style #main-wrapper #header-inner.stuck #main-nav > li > a,
					  .hb-special-header-style #main-wrapper #header-inner.stuck #close-fancy-search,
					  .hb-special-header-style #main-wrapper #header-inner.stuck #fancy-search input[type=text] { color: '.get_theme_mod('hb_spec_head_color_sticky_setting').' !important; }';

				$css .= '.hb-special-header-style #main-wrapper #header-inner.stuck #fancy-search ::-webkit-input-placeholder { color: '.get_theme_mod('hb_spec_head_color_sticky_setting').' !important; };
					  .hb-special-header-style #main-wrapper #header-inner.stuck #fancy-search ::-moz-input-placeholder { color: '.get_theme_mod('hb_spec_head_color_sticky_setting').' !important; };
					  .hb-special-header-style #main-wrapper #header-inner.stuck #fancy-search ::-ms-input-placeholder { color: '.get_theme_mod('hb_spec_head_color_sticky_setting').' !important; }';
			}
		}
		...
		// Minify the CSS code
		$css = hb_simple_minify_css( $css );

		// If file doesn't exist create one
		if ( ! file_exists( $css_dir . 'dynamic-styles.css' ) && wp_is_writable( $css_dir ) ) {
			touch( $css_dir . 'dynamic-styles.css' );
		}

		// Write to file and clean cache
		if ( wp_is_writable( $css_dir . 'dynamic-styles.css' ) && ! is_customize_preview() ) {
...
/**
 * Add dynamic CSS to wp_head if file is not writable or is customizer preview.
 * 
 * @since 3.4.2
 */
if ( ! function_exists( 'hb_dynamic_styles_output' ) ) {
	function hb_dynamic_styles_output() {
		if ( function_exists( 'hb_generate_dynamic_css' ) ) {
			echo '<style type="text/css">' . hb_generate_dynamic_css() . '</style>';
		}
	}
}

if ( ! wp_is_writable( get_theme_file_path ( 'assets/css/dynamic-styles.css' ) ) || is_customize_preview() ) {
	add_action( 'wp_head', 'hb_dynamic_styles_output', 101 );
}



/**
 * Simple minifying of CSS code
 * 
...
if ( ! function_exists( 'hb_simple_minify_css' ) ) {
	function hb_simple_minify_css( $css ) {
...
/**
 * Clean WordPress and external plugin cache
 * 
 * @since 4.0
 */
