...
if ( ! function_exists( 'hb_icon_picker' ) ) {
	function hb_icon_picker() {
		...
		<div id="hb-icons-modal" class="icons-loading" 
			data-uri-fontawesome="<?php echo get_parent_theme_file_uri( 'css/icons.css' ); ?>"<?php echo $vc_assets_uri; ?>>
...
if ( ! function_exists( 'hb_toolbar_link_to' ) ) {
	function hb_toolbar_link_to( $wp_admin_bar ) {
		...
		$args = array(
			...
			'title' 	=> '<span style="color:#f9b50f;">' . esc_html__( 'Get Expert Help', 'hbthemes' ) . '</span>',
			'href' 		=> 'http://www.anrdoezrs.net/links/8353661/type/dlg/http://www.mojomarketplace.com/item/wordpress-support',
			'parent' 	=> 'highend_link'
		);
		$wp_admin_bar->add_node( $args );
	}
